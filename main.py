...
# dataSet.exp[2].plt_chromatogram(ctype=['tic', 'bpc'])

# evaluate bpc
dataSet.get_bpc(plot=True)

# calculate similarity/dissimilarity (hclust)


# rt alignment based on warping
...
# clustering set A:
dataSet.exp[0].peak_picking(st_adj=6e2, q_noise=0.55, dbs_par={'eps': 0.0041, 'min_samples': 3}, selection=selection, plot=True)
dataSet.exp[1].peak_picking(st_adj=6e2, q_noise=0.55, dbs_par={'eps': 0.0041, 'min_samples': 3}, selection=selection, plot=True)
dataSet.exp[2].peak_picking(st_adj=6e2, q_noise=0.55, dbs_par={'eps': 0.0041, 'min_samples': 3}, selection=selection, plot=True)
...
st_adj=6e2
q_noise = 0.55
dbs_par = {'eps': 0.0041, 'min_samples': 3}
self=dataSet.exp[0]
# feature summary
